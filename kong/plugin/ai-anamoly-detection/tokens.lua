...
local function check_for_anomalies(conf, request_data)
    ...
    local gpt_payload = {
        ...
        messages = {
            {role = "system", content = "You are an AI that detects anomalies in API requests. If any anomalies are found append keyword anomaly found in response along with the type of anamoly. Ignore user agent. Don't put strict check."},
    ...
    local severity_score = tonumber(analysis_text:match("severity score: (%d+)"))
    ...
    if anomaly_status and anomaly_status == "none" then
        ...
        return false
    elseif anomaly_status and anomaly_status ~= "none" then
        kong.log.err("anomaly_status: ", anomaly_status)    
        return true
    end
